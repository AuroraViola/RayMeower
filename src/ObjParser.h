...
static inline struct Material *ImportMtl(const char *path, int *materialCount) {
    ...
    *materialCount = 0;
    ...
    while (fgets(current_line, sizeof(current_line), file) != NULL) {
        ...
        if (begins("illum", current_line) == 0) {
            int type;
            sscanf(current_line, "illum %d", &type);
            if (type != 3) {
                materials[(*materialCount)-1].metallic = 0;
            }
        }
        ...
        if (begins("map_Ns", current_line) == 0) {
            char filename[256];
            sscanf(current_line, "map_Ns %s", filename);
            char texturePath[256];
            ResolvePath(path, filename, texturePath);
            materials[(*materialCount)-1].roughnessMap = IMG_Load(texturePath);
        }
